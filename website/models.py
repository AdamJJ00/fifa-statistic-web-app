...
-- SQL query to retrieve top teams with highest market value
...
-- SQL query to retrieve the top 4 oldest teams (based on players age)
...
-- SQL query to retrieve the top 4 youngest teams (based on players age)
SELECT teams.team_name, ROUND(AVG((julianday('now') - julianday(players.date_of_birth))/365), 2) as average_age
FROM teams
JOIN players ON teams.team_id = players.team_id
GROUP BY teams.team_name
...
-- SQL query to retrieve top 4 teams with tallest players (on average)
...
-- SQL query to retrieve top 4 teams with shortest players (on average)
SELECT teams.team_name, ROUND(AVG(players.height_cm), 2) as height
FROM teams
JOIN players ON teams.team_id = players.team_id
GROUP BY teams.team_name
...
-- SQL query to retrieve top 10 youngest players
...
-- SQL query to retrieve top 10 oldest players
SELECT players.name, teams.team_name, players.date_of_birth as Date_of_birth
FROM players
JOIN teams ON teams.team_id = players.team_id
...
-- SQL query to retrieve the top 10 tallest players
...
-- SQL query to retrieve the top 10 shortest players
SELECT players.name, teams.team_name, players.position, players.height_cm as height
FROM players
JOIN teams ON teams.team_id = players.team_id
ORDER BY height ASC
...
-- SQL query to retrieve the schedule of matches for Group C
SELECT matches.Date, matches.Time, stadiums.Stadium_name, teams.Team_name as Home_team, a.Team_name as Away_team, matches.home_goals || ':' || matches.away_goals as Score
FROM matches JOIN teams ON matches.home_team_id=teams.team_id JOIN teams a ON matches.away_team_id=a.team_id JOIN stadiums ON stadiums.stadium_id = matches.stadium_id
...
-- SQL query to retrieve top 10 matches with most yellow/red cards
...
-- SQL query to retrieve all matches which took place at Lusail Stadium stadium
SELECT matches.Date, matches.Time, stadiums.Stadium_name, teams.Team_name as Home_team, a.Team_name as Away_team, matches.home_goals || ':' || matches.away_goals as Score
FROM matches JOIN teams ON matches.home_team_id=teams.team_id JOIN teams a ON matches.away_team_id=a.team_id JOIN stadiums ON stadiums.stadium_id = matches.stadium_id
WHERE stadiums.stadium_name = 'Lusail Stadium';

-- SQL query to retrieve all matches which took place at Al Bayt Stadium stadium
SELECT matches.Date, matches.Time, stadiums.Stadium_name, teams.Team_name as Home_team, a.Team_name as Away_team, matches.home_goals || ':' || matches.away_goals as Score
FROM matches JOIN teams ON matches.home_team_id=teams.team_id JOIN teams a ON matches.away_team_id=a.team_id JOIN stadiums ON stadiums.stadium_id = matches.stadium_id
